...
def _harvest_locked(display_name, nav, log):
    ...
    if display_name:
        try:
            docker_wx._exec("python3", "/usr/local/bin/wx_send.py", "open",
                            display_name, "0", timeout=40)   # 打开但不预滚,保留近期图片在视野
        except Exception as e:  # noqa: BLE001
            log(f"打开会话失败: {e}")
        time.sleep(1.0)

    opened = False
    for attempt in range(8):                  # 找照片→点开→进查看器(每屏试多个候选)
        try:
            im = _screenshot()
            pts = _find_photos(im)
        except Exception as e:  # noqa: BLE001
